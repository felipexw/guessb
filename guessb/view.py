...
def show_posts_2(request):
    ...
    __checkCookie(request, response)
    
    content = []
        
    try:
        factory = DAOFactory.getDAOFactory()
        content = factory.getGenericDAO(request.session.get('ACCESS_TOKEN')).getFeed()
    except:
        ...
        for i in xrange(0, len(content)):
            if i >= primeiro_indice and i < ultimo_indice:
                ...
                html += '<tr><td>' + img + '<p style="text-align: center">' + content[i]['authorName'] + '</p>  </td><td><p>' + content[i]['messageContent'] + '</p></td><td><p><a href="%s' % content[i]['link'] + '">Link</a></p></td><td><p><button type="button" onclick=%s' % st + ' role="button" class="btn btn-md btn-success"><i class="glyphicon glyphicon-tag icon-th"></i></a></p></td></tr>'
                j += 1
...
def show_posts(request):
    ...
    __checkCookie(response, request.GET.get('ACCESS_TOKEN'))
    
    factory = DAOFactory.getDAOFactory()
    
    dados = factory.getGenericDAO(__getCookie_(request)).getFeed()    
            
    j = 0
       
    for i in xrange(0, len(dados)):
        if i >= primeiro_indice and i < ultimo_indice:
            img = '<img  src="//graph.facebook.com/' + dados[i].get('authorId') + '/picture?type=large"' + ' style="width: 75px; height: 75px;" class="img-circle"/>'
            postId = "'%s'" % dados[i]['postId']
            st = "showCommentsFromPosts(%s,%s)" % ('0', postId)
            
            html += '<tr><td>' + img + '<p style="text-align: center">' + dados[i]['authorName'] + '</p>  </td><td><p>' + dados[i]['messageContent'] + '</p></td><td><p><a href="%s' % dados[i]['link'] + '">Link</a></p></td><td><p><button type="button" onclick=%s' % st + ' role="button" class="btn btn-md btn-success"><i class="glyphicon glyphicon-tag icon-th"></i></a></p></td></tr>'
            j += 1
        if j == QUANTIDADE_PUBLICACOES_PAGINA:
            break        
...
def show_comentarios(request):
    ...
    try:
        factory = DAOFactory.getDAOFactory()
        content = factory.getGenericDAO(request.session.get('ACCESS_TOKEN')).getCommentsFeed(request.GET.get('postId'))
    except:
        html = '<div id="jumbotron" class="jumbotron"> <p>Sua sessão no Facebook expirou. Por favor, conecte-se novamente.</p></div>'
    else:
        j = 0
        ...
        html += __get_html_paginacao(len(content), QUANTIDADE_PUBLICACOES_PAGINA, "'comments'", request.GET.get('postId'), request.GET.get('page'))
